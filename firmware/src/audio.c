...
static volatile TaskHandle_t file_task_handle = NULL;
...
esp_err_t audio_play_pcm(const int16_t *data, size_t samples) {
    ...
    size_t written = 0;
    return i2s_channel_write(tx_chan, data, samples * sizeof(int16_t), &written, pdMS_TO_TICKS(100));
}
...
esp_err_t audio_mic_read(int16_t *buf, size_t samples, size_t *bytes_read) {
    ...
    return i2s_channel_read(rx_chan, buf, samples * sizeof(int16_t), bytes_read, pdMS_TO_TICKS(100));
}
...
static void file_play_task(void *arg) {
    ...
    while (file_playing) {
        ...
        uint8_t *data = (uint8_t *)xRingbufferReceiveUpTo(
            file_ringbuf, &item_size, pdMS_TO_TICKS(200), AUDIO_FRAME_BYTES);
        if (data && item_size > 0) {
    ...
    // 先清 handle 再删 task，避免竞态
    TaskHandle_t h = (TaskHandle_t)file_task_handle;
...
esp_err_t audio_file_play_start(void) {
    ...
    file_playing = true;
    xTaskCreate(file_play_task, "file_play", 8192, NULL, 5, (TaskHandle_t *)&file_task_handle);
    return ESP_OK;
}

void audio_file_play_stop(void) {
    file_playing = false;
    // 等 task 自己退出，最多 500ms
    for (int i = 0; i < 50 && file_task_handle != NULL; i++) {
